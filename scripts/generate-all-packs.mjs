/**
 * Writes all translation-packs/*.mjs with full native section blocks.
 * Run: node scripts/generate-all-packs.mjs
 */
...
/** @type {Record<string, { title: string; description: string; tags: string[]; alt: string; related: string; closing: string; main: string[] }>} */
const PACKS = {
  fr: {
    title: "Seedance 2.0 : analyse complète des entrées multimodales",
    description:
      "Texte, image, audio et vidéo dans Seedance 2.0, système @ et jusqu'à 12 fichiers de référence. Tutoriel Seedance pour la vidéo IA multimodale.",
    tags: ["Seedance 2.0", "Tutoriel Seedance", "Version chinoise Seedance"],
    alt: "Seedance 2.0 : analyse complète des entrées multimodales",
    related:
...
![Seedance 2.0 : analyse complète des entrées multimodales](${IMG})
...
// Import remaining packs from data file if present
const dataPath = path.join(__dirname, "all-packs-extra.json");
if (fs.existsSync(dataPath)) {
  const extra = JSON.parse(fs.readFileSync(dataPath, "utf8"));
  Object.assign(PACKS, extra);
}

for (const [lang, pack] of Object.entries(PACKS)) {
  if (lang === "de") continue;
  writePackFile(lang, pack, outDir);
  console.log("Wrote pack", lang);
}

console.log("Packs ready:", Object.keys(PACKS).join(", "));
